...
plotly::ggplotly(ggplot(data = plot_missing_data,aes_string(x="file_name",y="missing_data",color=params$color_by))+geom_point()+
...
plotly::ggplotly(ggplot(data = plot_missing_data,aes_string(x="file_name",y="tic_data",color=params$color_by))+geom_point()+
...
plotly::ggplotly(ggplot(data = dt_fr,aes_string(x="value",y="dev",color=params$color_by))+geom_point()+
...
all_charges<-sort(unique(unlist(sapply(charge_data,names))))
all_charges_U<-sort(unique(unlist(sapply(charge_data,names))))
all_charges<-data.frame(sample=column_names_cmp[["charge"]],
                        ...
                        do.call(rbind, replicate(length(column_names_cmp[["charge"]]), data_exp_design, simplify=FALSE)))
...
plotly::ggplotly(ggplot(data=all_charges, aes_string(x="sample", y="count_qc",color=params$color_by)) +  geom_point()+
...
charge_data<-apply(data_abundance[,column_names_cmp[["charge"]]],1,function(x){(table(x)/length(x))*100})


all_charges<-sort(unique(unlist(sapply(charge_data,function(x){names(x[x==100])}))))
...
```{r, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE,include=print_qc_standards,eval=print_qc_standards}
 ...
 knitr::asis_output("## Internal standard-based QC
...
if(params$id_use_rt)
{
 ...
 knitr::asis_output( paste( "The total number of identified metabolites is ",length(intersect(unique_ids_database,unique_ids_exp)), " out of ",length(unique_ids_database),". In addition, ",length(unique(data_identification[duplicated(data_identification[,"Met_ID"]),"Met_ID"]))," metabolites were not assigned to a unique feature.
Finally, ", length(unique(ids_exp[duplicated(ids_exp)]))," metabolites were assigned to multiple features.")) 
 

knitr::asis_output("### QC based on Identification")
...
knitr::asis_output("### Missing values")
knitr::asis_output("Percentage of missing values compared to the total number of features find in the *entire* dataset")
...
(plotly::ggplotly(ggplot(data = plot_point_data,aes_string(x="file_name",
                                                          y="points",color=params$color_by))+geom_point()+
...
plotly::ggplotly(ggplot(data = mz_hist,aes_string(x="x"))+geom_histogram(aes(y=..density..), colour="black", fill="white")+
 geom_density(alpha=.2, fill="blue") +theme_classic()+xlab("ppm"))
